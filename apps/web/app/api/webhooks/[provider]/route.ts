...
const providers: Record<string, ProviderHandler> = {
  ...
  stripe: async (req: NextRequest) => {
    ...
    const { handleStripe } = await import('@/src/server/webhooks/stripe');
  ...
  n8n: async (req: NextRequest) => {
    ...
    const signature = req.headers.get('X-Hikari-Signature');
    const timestamp = req.headers.get('X-Hikari-Timestamp');
